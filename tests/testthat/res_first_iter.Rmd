...
scalar_iid <- update_prior_covariance_scaled_iid(X, U$U0, V, p, minval)
U_scalar_iid <- update_prior_covariance_scaled_struct(U, scalar_iid)

r <- sum(eigen(U$U0)$values > 1e-15)
scalar_general <- update_prior_covariance_scaled_fa_general(X, U$U0, V.g, p, U$s, r)
U_scalar_general <- update_prior_covariance_scaled_struct(U, scalar_general)

save(U_scalar_iid, file = 'scalar_iid.rds')
save(U_scalar_general, file = 'scalar_general.rds')
