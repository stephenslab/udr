...
#' prior on U. U \sim W_R^{-1}(S0, \nu), where \nu = n0-R-1. 
#' The derivation for U is available in overleaf eq.(33).
#' https://www.overleaf.com/read/vrgwpskkhbpj
ed_reg_iid <- function(X, U, p, S0, n0){
  m  <- ncol(X)
  I  <- diag(m)
  A  <- solve(U + I,U)
  B <- U %*% (I-A)
  bmat <- X %*% A
  U <- (crossprod(sqrt(p)*bmat)+sum(p)*B+n0*S0)/(sum(p)+ n0)
  return(U)
}


#' Function to compute log posterior. 
compute_log_posterior_ed <- function(X, w, U, V, S0, n0){
  log_prior <- 0
  K <- length(w)
  for (k in 1:K){
    log_prior = log_prior + ldiwishart(W = (U[,,k]+t(U[,,k]))/2, n0, S0)
  }
...
em_fit_regularized_ed <- function(X, w, U, V, S0, n0, maxiter, tol, tol.obj){
  ...
  for (iter in 1:maxiter){
    ...
    for (j in 1:k){
      U[,,j] = ed_reg_iid(X, U[,,j], P[,j], S0, n0)
    }
    # update mixture weight
    w = colSums(P)/n
    # Compute log-likelihood.
    loglik = loglik_ud_iid_helper(X, w, U, V)
    log_posterior = compute_log_posterior_ed(X, w, U, V, S0, n0)
