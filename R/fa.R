...
# update_prior_covariance_unconstrained_fa_iid. (The C++ version has
# not yet been implemented, so for now we simply all the R
# implementation.)
...
# Perform an M-step update for unconstrained prior covariance (U)
# using factor analysis, allowing for different residual variances V
# among the samples.
...
# using factor analysis for the special case when V = I for all
# samples; that is, the model is x ~ N(0,sU + I).
update_prior_covariance_scaled_fa_iid <- function (X, U, p, ...) {
  r <- sum(eigen(U$U0)$values > 1e-15)
  return(update_prior_covariance_struct_scaled(U,fa_scaled_iid(X,U$U0,p,U$s)))
}

# This is a more efficient C++ implementation of
# update_prior_covariance_scaled_fa_iid. (The C++ version has not yet
# been implemented, so for now we simply all the R implementation.)
...
# Perform an M-step update for a scaled prior covariance (U) using
...
update_prior_covariance_scaled_fa_notiid <- function (X, U, V, p, ...) {
  r <- sum(eigen(U$U0)$values > 1e-15)
  return(update_prior_covariance_struct_scaled(U,fa_scaled_notiid(X,U$U0,V,
                                                                  p,U$s)))
}
...
# This is a more efficient C++ implementation of
# update_prior_covariance_rank1_fa_iid. (The C++ version has not yet
# been implemented, so for now we simply all the R implementation.)
update_prior_covariance_rank1_fa_iid_rcpp <- function (X, U, p, ...) {
  message("update_prior_covariance_rank1_fa_iid_rcpp is not yet ",
          "implemented; using R version instead")
...
update_prior_covariance_rank1_fa <- function (X, U, V, p, ...) { 
  if (is.matrix(V)) {
    n   <- nrow(X)
    m   <- ncol(X)
    V   <- array(V,c(m,m,n))
...
fa_scaled_notiid <- function (X, U0, V, p, s) {
  ...
  for (i in 1:n) {
    ...
    VinvQ[[i]] = solve(V[,,i]) %*% Q
    Sigma[[i]] = solve(crossprod(Q, VinvQ[[i]]) + I/s)
