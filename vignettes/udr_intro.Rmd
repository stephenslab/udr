---
title: "Introduction to Ultimate Deconvolution"
author: Peter Carbonetto
date: "`r Sys.Date()`"
output:
  html_document:
    theme: readable
vignette: >
  %\VignetteIndexEntry{Introduction to Ultimate Deconvolution}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r knitr-opts, include=FALSE}
knitr::opts_chunk$set(comment = "#",collapse = TRUE,results = "hold",
                      fig.align = "center")
```

Here we illustrate the features of the Ultimate Deconvolution (UD)
package.

```{r load-pkgs}
library(udr)
```

We simulate 4,000 data points from a UD model with 4 mixture
components:
$$
x_i \sim w_1 N(0,V + U_1) + \cdots + w_4 N(0,V + U_4).
$$

```{r sim-data-settings}
set.seed(1)
n <- 4000
V <- rbind(c(0.8,0.2),
           c(0.2,1.5))
U <- list(none   = rbind(c(0,0),
                         c(0,0)),
          shared = rbind(c(1.0,0.9),
                         c(0.9,1.0)),
          only1  = rbind(c(1,0),
                         c(0,0)),
          only2  = rbind(c(0,0),
                         c(0,1)))
w <- c(0.8,0.1,0.075,0.025)
X <- simulate_ud_data(n,w,U,V)
```

This is the most basic usage:

```{r ud-fit-1}
fit0 <- ud_init(X)
fit1 <- ud_fit(fit0)
```

The default model is a mixture with a total of $K = 10$ mixture
components with 2 scaled, 4 rank-1, and 4 unconstrained prior
covariance matrices $U_k$:

```{r ud-fit-2}
summary(fit1)
```

The function `ud_init` is a flexible interface for design a variety of
UD models. For example,

```{r ud-fit-3}
fit0 <- ud_init(X,U_scaled = U,n_rank1 = 1,n_unconstrained = 1,V = V)
fit2 <- ud_fit(fit0)
```

The `ud_fit` interface also allows for flexibly re-fitting a model;
that is, a `ud_fit` output can be used in another call to `ud_fit`.

Notice that the data do not need to be passed to `ud_fit` because they
are already embedded in the `ud_fit` output. However, one can pass a
new data set `X` to `ud_fit`.

The `control` argument to `ud_fit` controls the optimization of the
model parameters. For example, the unconstrained prior covariance
matrices are by default optimized using the Extreme Deconvolution (ED)
updates, and we can replace these with the new "truncated eigenvalue"
EM (TEEM) updates:

```{r ud-fit-4}
fit3 <- ud_fit(fit0,control = list(unconstrained.update = "teem"))
```

Notice that the likelihood is slightly higher with the TEEM updates,
reflecting the fact that the TEEM updates have greater freedom to
optimize the covariance matrices.

These are the current model fitting defaults:

```{r ud-fit-control-default}
unlist(ud_fit_control_default())
```

The most intensive computations are implemented in R (`version = R`)
and C++ (`version = Rcpp`); both versions should return the same, or
nearly the same output, but the C++ version can be much faster.

The default updates for the scaled and rank-1 prior covariance
matrices are `"none"`; they have not yet been implemented.

In some cases each sample might have different measurement error, and
udr can handle such cases by specifying `V` as a list instead of a matrix:

```{r ud-fit-5}
fit0 <- ud_init(X,V = rep(list(V),n))
```

However, the ED update has not yet been implemented for this case.
(The TEEM update is not applicable to this case).

```{r ud-fit-6}
fit5 <- ud_fit(fit0,control = list(unconstrained.update = "none"))
```

Session info
------------

This is the version of R and the packages that were used to generate
these results.

```{r session-info}
sessionInfo()
```
